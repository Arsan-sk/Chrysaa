...
export function SampleWorksCarousel() {
  ...
  // Calculate position styles for coverflow layout with responsive spacing and vertical centering
  const getCardStyle = (index: number) => {
    ...
    // Responsive step: 220px on mobile vs 320px on desktop
    ...
    >
      ...
      <div className="sample-stage-3d">
        {sampleWorks.map((work, index) => {
          ...
          const imageSrc = work.image ? work.image.replace(/^\/public/, "") : "";
          const hasImageError = Boolean(imageErrorMap[work.id]);
              ...
              <div className="sample-card-frame">
                ...
                <div className="sample-card-bar">
                  ...
                  <span className="sample-card-url">
                    {work.href ? work.href.replace(/^https?:\/\//, "").replace(/\/$/, "") : `${work.id}.chrysa.systems`}
                  </span>
                ...
                {/* Card Media Preview with High-Fidelity Fallback */}
                <div className="sample-card-media">
                  {/* Real Image Preview when available and not errored */}
                  {imageSrc && !hasImageError ? (
                    ...
                    /* Base Mockup / Graceful Fallback (Rendered when image not available or errored) */
                    <div
                      className="sample-mockup-fallback"
                      style={{
                        background: `linear-gradient(145deg, #151412 0%, #1c1a17 50%, #131210 100%)`,
                        borderBottom: `2px solid ${work.accentColor || "#E8A33D"}`,
                      }}
                    >
                      <div className="mockup-decor-grid" />
                      <div
                        className="mockup-ambient-glow"
                        style={{ background: `${work.accentColor}25` }}
                      />
                      <div className="mockup-center-icon">
                        <div
                          className="mockup-icon-wrapper"
                          style={{
                            background: `radial-gradient(circle, ${work.accentColor}30 0%, rgba(20,19,16,0.85) 80%)`,
                            borderColor: `${work.accentColor}60`,
                            boxShadow: `0 0 20px ${work.accentColor}30`,
                          }}
                        >
                          <Sparkles size={24} color={work.accentColor || "#E8A33D"} />
                        </div>
                        <span className="mockup-tag" style={{ color: work.accentColor }}>
                          {work.category}
                        </span>
                        <h4 className="mockup-title">{work.title}</h4>
                        {work.stats && (
                          <span className="mockup-subtag">
                            {work.stats}
                          </span>
                        )}
                      </div>
                ...
                <div className="sample-card-content">
                  <div className="sample-card-top-row">
                    <span className="sample-card-category" style={{ color: work.accentColor }}>
                      {work.category}
                    </span>
      ...
      {/* Navigation Controls Bar — Clearly positioned AFTER the carousel */}
